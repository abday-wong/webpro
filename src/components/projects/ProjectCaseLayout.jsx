...
const TechBadge = ({ children }) => (
  <span className="px-3 py-1.5 bg-neutral-900 font-mono text-[10px] md:text-xs font-bold uppercase tracking-[0.12em] md:tracking-[0.16em] text-white/80">
    {children}
  </span>
);
      ...
      <div className="sticky top-0 z-40 bg-[#030303]/90 backdrop-blur-md border-b border-white/10">
        <div className="max-w-6xl mx-auto px-6 md:px-10 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-[10px] uppercase font-bold tracking-[0.12em] md:tracking-[0.16em] text-white/30 border-l border-white/15 pl-3">
              {project.year}
            </span>
          </div>
          <button
            onClick={onClose}
            className="flex items-center gap-2 rounded-full border border-white/15 bg-neutral-900 px-5 py-2 text-xs md:text-sm font-bold tracking-wide uppercase hover:bg-white hover:text-black transition-all duration-300 shadow-sm hover:shadow-md"
          >
        ...
        <section className="max-w-4xl mx-auto text-center flex flex-col items-center px-6 md:px-10 pt-16 md:pt-20 pb-12">
          ...
          >
            {project.title.split(' ').map((word, i, arr) => (
              <span key={i} className={i === arr.length - 1 ? "text-transparent" : ""} style={i === arr.length - 1 ? { WebkitTextStroke: '2px rgba(255,255,255,0.6)' } : {}}>
                {word}{i !== arr.length - 1 ? ' ' : ''}
              </span>
            ))}
          </Gsap.h1>
          ...
          >
            ...
            {project.links?.repo && (
              <a href={project.links.repo} target="_blank" rel="noreferrer"
                className="bg-neutral-900 text-white px-8 py-3.5 font-bold uppercase text-xs tracking-wider border border-transparent hover:border-white/15 transition-all duration-300 rounded-[2px] flex items-center gap-2">
        ...
        {project.heroImg && (
          <section className="px-6 md:px-10 pb-16">
            <Gsap.div
              ...
              className="max-w-5xl mx-auto border border-white/10 p-2 bg-neutral-900 rounded-lg shadow-sm"
            >
              <div className="w-full bg-neutral-950/50 rounded-[4px] overflow-hidden aspect-video relative flex items-center justify-center border border-white/10">
                <img
        ...
        <section className="max-w-5xl mx-auto px-6 md:px-10 pb-24">
          ...
          <div className="grid md:grid-cols-2 gap-12 lg:gap-20 mt-16 border-t border-white/15 pt-16">
              ...
              {project.features && project.features.length > 0 && (
                <div>
                  <h2 className="text-xs font-mono font-bold uppercase tracking-[0.12em] md:tracking-[0.16em] text-white/40 mb-6 flex items-center gap-3">
                    <span className="w-6 h-[1px] bg-white/20" /> Key Features
                  </h2>
            ...
            <div className="space-y-16">
              {project.impact && project.impact.length > 0 && (
                <div>
                  <h2 className="text-xs font-mono font-bold uppercase tracking-[0.12em] md:tracking-[0.16em] text-white/40 mb-6 flex items-center gap-3">
                    <span className="w-6 h-[1px] bg-white/20" /> Outcome & Impact
                  </h2>
                  <ul className="space-y-4 text-base leading-relaxed text-white/80">
          ...
          <div className="mt-12 lg:mt-16 border-t border-white/15 pt-12 lg:pt-16">
            {project.stack && project.stack.length > 0 && (
              <div>
                <h2 className="text-xs font-mono font-bold uppercase tracking-[0.12em] md:tracking-[0.16em] text-white/40 mb-6 flex items-center gap-3">
                  <span className="w-6 h-[1px] bg-white/20" /> Tech Stack
                </h2>
